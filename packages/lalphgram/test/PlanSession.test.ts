...
describe("PlanSession", () => {
  ...
  it.live(
    "emits PlanTaskCreationStarted on [Session started] and suppresses other stage 2 text",
    () =>
      Effect.gen(function*() {
        ...
        yield* Effect.gen(function*() {
          ...
          const arr = [...events]
          const taskCreationEvents = arr.filter((e) => e._tag === "PlanTaskCreationStarted")
          expect(taskCreationEvents).toHaveLength(1)
          expect(taskCreationEvents[0]).toBeInstanceOf(PlanTaskCreationStarted)
          const textEvents = arr.filter((e) => e._tag === "PlanTextOutput")
