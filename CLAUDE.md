...
- **Pre-commit hooks (husky)** run `pnpm check`, `pnpm lint-staged`, and `pnpm test` automatically. If a hook fails, fix the reported errors and retry the commit — never skip or bypass hooks
